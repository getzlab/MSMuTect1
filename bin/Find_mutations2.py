...
def log_likelihood(reads,A,f,P):
	L_k_log=0;
	for k in np.arange(reads.size/2):
...
LOR_ratio=float(sys.argv[4])
p_equal=float(sys.argv[5])
KS_thresh=float(sys.argv[6])
#print p_equal
count_mut=0;count=0;
for Norm_t in f1:
	Tum_t=f2.readline()
	...
	Norm_list=Norm_t.split(" -999 ");
	Norm_list[3].strip('\n')
	Tum_list=Tum_t.split(" -999 ");
